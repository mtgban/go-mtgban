...
error names the card by id alone. An `ArbitEntry` carries no datastore of its
own: `String` names the card by the id it was keyed on, and a nil backend
passed to a CSV function is an error naming the function rather than a
silent resolution of nothing.
