...
**Four scrapers convert but stay unregistered.** `cardsphere`, `mtgstocks`,
`secretdeskorrigans` and `toamagic` all took a `*Backend` first like every
other package, but none gets a `register.go`: none of the four had a
bantool target before this series either, and registering one now would
demand a `bantool-<store>.yml` and a `game` input for a target that has
never been scheduled and has nothing to schedule it. They are library-only
on purpose, not half-migrated:
...
one, so leaving these four out asks nothing of it.

**mtgban's own readers take the backend.** `Arbit`, `Mismatch`,
`Pennystock` and the CSV loaders and writers all take it as their first
parameter; `ArbitOpts.Backend` is gone, and `ArbitOpts` is left as what it
always was underneath, a bag of optional filters. A nil backend is refused
where a nil one is meaningless: the three reports return nil, and a CSV
function returns an error naming itself, rather than either resolving
nothing silently. The duplicate-entry error names the card by id alone. An
`ArbitEntry` carries no datastore of its own and no longer formats itself —
it is data, and the CSV writers are the one path in the package that turns
an id into a name, through the backend they were handed.
...
loads and calls methods on it, passes it as the first argument to the reports
and to `WriteBuylistToCSV`. It pins a released version, so nothing changes for
it until then.
...
- [x] Every registered scraper package converted; bantool built from the
      registry.
