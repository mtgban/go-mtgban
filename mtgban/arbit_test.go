...
// backendFor builds a *mtgmatcher.Backend around a fixed set of cards. Every
// arbitrage function reads the card to filter on it, so a card the backend
// does not hold is skipped outright and the arithmetic below would never run.
func backendFor(cards map[string]*mtgmatcher.CardObject) *mtgmatcher.Backend {
	return &mtgmatcher.Backend{UUIDs: cards}
}

// withBackend returns a copy of opts (or a zero ArbitOpts, when opts is nil)
// with its Backend set, so a table of options shared across subtests is
// never mutated in place.
func withBackend(opts *ArbitOpts, b *mtgmatcher.Backend) *ArbitOpts {
	if opts == nil {
		return &ArbitOpts{Backend: b}
	}
	o := *opts
	o.Backend = b
	return &o
...
func TestArbitReportsTheTrade(t *testing.T) {
	...
	entries := Arbit(&ArbitOpts{Backend: b}, vendor, seller)
	if len(entries) != 1 {
...
func TestArbitDoesNotScaleASingleCopy(t *testing.T) {
	...
	entries := Arbit(&ArbitOpts{Backend: b},
...
func TestArbitFilters(t *testing.T) {
	...
	for _, tt := range []struct {
		...
		t.Run(tt.desc, func(t *testing.T) {
			b := backendFor(plainCard())
			vendor, seller := base()
			if got := len(Arbit(withBackend(tt.opts, b), vendor, seller)); got != tt.want {
...
func TestArbitKeepsAQuantitylessSeller(t *testing.T) {
	...
	entries := Arbit(&ArbitOpts{Backend: b, MinQuantity: 5},
		vendorOf(BuylistRecord{"card": {{Conditions: "NM", BuyPrice: 15}}}),
...
func TestArbitAppliesTheRateAndFactors(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.desc, func(t *testing.T) {
			...
			entries := Arbit(withBackend(tt.opts, b),
				vendorOf(BuylistRecord{"card": {{Conditions: "NM", BuyPrice: 15}}}),
				sellerOf(InventoryRecord{"card": {{Conditions: "NM", Price: 10, Quantity: 1}}},
					ScraperInfo{Name: "seller"}))
			if len(entries) != 1 {
...
func TestArbitCardFilters(t *testing.T) {
	for _, tt := range []struct {
		...
		t.Run(tt.desc, func(t *testing.T) {
			...
			got := Arbit(withBackend(tt.opts, b),
				vendorOf(BuylistRecord{"card": {{Conditions: "NM", BuyPrice: 15}}}),
				sellerOf(InventoryRecord{"card": {{Conditions: "NM", Price: 10, Quantity: 1}}},
					ScraperInfo{Name: "seller"}))
			if len(got) != tt.want {
...
func TestArbitRechecksTheBuyFloorPerCondition(t *testing.T) {
	...
	entries := Arbit(&ArbitOpts{Backend: b, MinBuyPrice: 10},
