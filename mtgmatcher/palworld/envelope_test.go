...
func TestLoadAcceptsMetaDataEnvelope(t *testing.T) {
	legacy, err := Load(strings.NewReader(tailFixture))
	if err != nil {
		t.Fatal(err)
	}

	var payload map[string]any
	if err := json.Unmarshal([]byte(tailFixture), &payload); err != nil {
		t.Fatal(err)
	}
	delete(payload, "game")
	...
	envelope, err := json.Marshal(map[string]any{
		"meta": map[string]any{"date": "2026-09-14", "version": "1", "game": "palworld"},
