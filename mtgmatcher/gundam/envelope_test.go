...
func TestLoadAcceptsMetaDataEnvelope(t *testing.T) {
	legacy, err := Load(strings.NewReader(qualifiedFixture))
	if err != nil {
		t.Fatal(err)
	}

	var payload map[string]any
	if err := json.Unmarshal([]byte(qualifiedFixture), &payload); err != nil {
		t.Fatal(err)
	}
	delete(payload, "game")
	...
	envelope, err := json.Marshal(map[string]any{
		"meta": map[string]any{"date": "2026-09-14", "version": "1", "game": "gundam"},
