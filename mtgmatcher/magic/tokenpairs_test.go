...
func TestMintVerifiedPairsResolves(t *testing.T) {
	...
	id := MatchTokenPairingByUUIDs(testBackend, angel[0].UUID, demon[0].UUID, false)
	if id == "" {
...
func TestVerifiedPairCollisionRefusesRatherThanGuess(t *testing.T) {
	...
	if id := MatchTokenPairingByUUIDs(testBackend, bird[0].UUID, realWizard[0].UUID, false); id == "" {
	...
	if id := MatchTokenPairingByUUIDs(testBackend, bird[0].UUID, verifiedWizard[0].UUID, false); id == "" {
	...
	if id, found := testBackend.TokenPairIndex[bird[0].UUID]["wizard"]; found {
