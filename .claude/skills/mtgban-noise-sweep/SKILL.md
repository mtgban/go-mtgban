...
The job is to turn each bad listing into either a landed price or a stated
reason it cannot land. Guessing at a rule from an error message alone produces
rules that fire on nothing; the loop below is what actually moves the number.
...
2. **Cross-vendor spread** on the published site: capture `/arbit`, `/reverse`
   ...
   parses to nothing (`references/detectors.md`).
...
Mechanics for both in `references/detectors.md`.
...
the next session can re-run it. Templates: `references/harnesses.md`.
...
work; see `references/harnesses.md`.
