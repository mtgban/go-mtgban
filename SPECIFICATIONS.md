...
- **bantool** — a table of `scraperOption{flags}` in `cmd/bantool/main.go`
  for every target, well past a hundred once every scraper's own per-game and
  `_sealed` variants are counted (14 `*_riftbound` targets, 14 `*_lorcana`
  ones, and the rest of the eight non-Magic games besides). bantool builds no
  scrapers of its own: `targets()` walks `mtgban.AllGames` ×
  `mtgban.Registered(game)`, each scraper package having filed its own keys
  with `mtgban`'s registry from `init`, so an entry here is a name and its
  flags and nothing else. The table is a
