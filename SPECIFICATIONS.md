...
**Deprecated shims.** `mtgmatcher/deprecated.go` re-declares
`PromoTypeBoosterfun`, `PromoTypeBuyABox`, `PromoTypePrerelease`,
`PromoTypePromoPack` and `PromoTypeThickDisplay` so that `mtgban-website`
keeps compiling against core until it migrates to `mtgmatcher/magic`. They are
re-declared rather than aliased for a structural reason: core cannot import
`magic` without creating an import cycle. Do not add to this file; new code
takes the constants from the `magic` package.
