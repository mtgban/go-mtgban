...
func TokenPairAnchorUUIDs(bp Blueprint) (uuidA, uuidB string, ok bool) {
	...
	faces := [2]string{magic.StripFaceWrapping(first), magic.StripFaceWrapping(second)}

	anchorOne := func(numbers [2]string) (string, string, bool) {
		var uuids [2]string
		for i, face := range faces {
			cards := mtgmatcher.MatchInSetNumber(face, tokenSet, numbers[i])
			if len(cards) != 1 {
				return "", "", false
			}
			uuids[i] = cards[0].UUID
		}
